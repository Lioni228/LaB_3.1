...
1. Git clone ````https://github.com/Lioni228/LaB_3.1.git````
...
* Первая кнопка ````Сохранённые данные```` перекидывает на окно со списком данных.
* Вторая кнопка ````Внести запись```` перекидывает на окно со строчкой ввода «ФИО» и кнопкой «Сохранить запись».
